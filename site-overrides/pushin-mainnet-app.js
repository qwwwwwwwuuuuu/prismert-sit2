(function(){
  ...
  function mount(){
    ...
    <main class="pm-main"><p class="pm-label">PUSHIN / MAINNET PRE-LAUNCH</p><h1>Wallet and session</h1><p>Real wallet reads on Robinhood Chain. PUSHIN token trading has not launched.</p><p data-network role="status"></p><p data-error role="alert"></p>
    ...
    document.querySelector('.pm-main').insertAdjacentHTML('beforeend', '<section class="pa-card pm-account"><h2>From pre-view to a position</h2><ol><li>Pre-view: inspect charts and market specifications.</li><li>Connect a wallet: optional account and balance reads.</li><li>Start a practice session in the terminal.</li><li>Use virtual collateral to open and close positions.</li></ol><a class="pa-button" href="/trade.html">Open terminal</a></section><section class="pa-card pm-account"><h2>Session key scope</h2><p>The protocol design delegates order signing to a scoped key. PUSHIN practice trading currently uses a browser-local account instead.</p><h3>It can</h3><p>Store virtual orders, positions and history on this device.</p><h3>It can never</h3><p>Transfer real tokens or approve a spending allowance.</p></section><section class="pa-card pm-account"><h2>What you sign, and what you do not</h2><p>This wallet page requests account access and may request a network switch. It does not request an order signature, token approval or deposit transaction.</p><a href="/docs/eip712.html">Read the session-key specification</a></section>');
