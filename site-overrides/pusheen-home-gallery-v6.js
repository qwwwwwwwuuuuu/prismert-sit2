(() => {
  ...
  const placeSideArtwork = (container) => {
    ...
    container.querySelectorAll('h2').forEach(heading => {
      ...
      room.style.setProperty('--pushin-side-image', `url("/brand/${scene === "settlement" ? "pusheen-settlement-clean-v2.webp" : scene === "docs" ? "pusheen-docs-clean-v4.webp" : `pusheen-${scene}-cutout.png`}")`);
